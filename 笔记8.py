...
var[-1: 0: -2] # db
s.str[0] # 0    a, 1    e, 2    h
s.str[-1: 0: -2] # 0    db, 1     g, 2    i，注意这里的-1:0:-2，不包括0
s.str[2] # 0    c, 1    g, 2    NaN
s = pd.Series([{1: 'temp_1', 2: 'temp_2'}, ['a', 'b'], 0.5, 'my_string'])
s.str[1] # 0    temp_1, 1    b, 2    NaN, 3    y，四个元素，访问每个的第二个元素
s.astype('string').str[1]
# 0    1, 1    ', 2    ., 3    y，四个元素，访问每个的第二个元素，注意这里的类型转换

s = pd.Series(['a'])
s.str.len() # 0    1，字符串长度,访问的是每个元素的长度
s.astype('string').str.len() # 0    1，字符串长度
s == 'a' # 0    True，，dtype: bool
s.astype('string') == 'a' # 0    True，，dtype: boolean

s = pd.Series(['a', np.nan]) # 带有缺失值
s.str.len() # 0    1.0, 1    NaN
s.astype('string').str.len() # 0    1, 1    <NA>
s == 'a' # 0    True, 1 False, dtype: bool
s.astype('string') == 'a' # 0    True, 1 <NA>, dtype: boolean

# 对于全体元素为数值类型的序列，即使其类型为`object`或者`category`也不允许直接使用`str`属性
# 需要先转换为`string`类型
...
# 正则表达式
import re
re.findall(r'Apple', 'Apple! This Is an Apple!') # ['Apple', 'Apple']
''' 
元字符：.      匹配除换行符以外的任意字符
元字符：[ ]    字符集，匹配方括号中包含的任意字符
元字符：[^ ]   否定字符集，匹配方括号中不包含的任意字符
元字符：*      匹配前面的子表达式零次或多次
元字符：+      匹配前面的子表达式一次或多次
元字符：?      匹配前面的子表达式零次或一次
元字符：{n,m}  花括号,匹配前面字符至少n次,但是不超过m次
元字符：(xyz)  字符组,按照确切的顺序匹配字符xyz
元字符：|      分支结构,匹配符号之前的字符或后面的字符
元字符：\      转义符,它可以还原元字符原来的含义,允许你匹配保留字符[ ] ( ) { } . * + ? ^ $ \ |
元字符：^      匹配行的开始
元字符：$      匹配行的结束
'''
re.findall(r'.', 'abc') # r表示原生字符串，不需要转义，['a', 'b', 'c']
re.findall(r'[ac]', 'abc') # ['a', 'c']
re.findall(r'[^ab]', 'abc') # ['c']
re.findall(r'[ab]{2}', 'aaaabbbb') # {n}指匹配n次，['aa', 'aa', 'bb', 'bb']
